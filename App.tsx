    ...
    reader.onload = (event) => {
      try {
        ...
        const headerRowIndex = rows.findIndex((row) =>
          row.some((cell) => normalizeHeader(cell) === 'divisao')
        );

        if (headerRowIndex < 0) {
          reject(new Error('Cabeçalho não encontrado na primeira aba.'));
          return;
        }

        const headers = rows[headerRowIndex].map(normalizeHeader);
        const requiredMissing = REQUIRED_COLUMNS.filter(
          (col) => !headers.includes(normalizeHeader(col))
        );

        if (requiredMissing.length) {
          reject(
            new Error(`Faltam colunas obrigatórias: ${requiredMissing.join(', ')}`)
          );
          return;
        }
        ...
        const parsed: PlantingRow[] = rows
          ...
          .filter(
            (row) =>
              ...
              row.areaTotal > 0 &&
              row.areaPlantada >= 0 &&
              row.dataPlantio instanceof Date &&
              !Number.isNaN(row.dataPlantio.getTime())
          );
...
function summarizeFields(rows: PlantingRow[]): FieldSummary[] {
  ...
  return Array.from(byField.entries()).map(([key, values]) => {
    const ordered = [...values].sort(
      (a, b) => a.dataPlantio.getTime() - b.dataPlantio.getTime()
    );
...
function buildAlertPeriods(points: DemandPoint[]): AlertPeriod[] {
  ...
  points.forEach((point) => {
    const status: AlertPeriod['status'] =
      point.backlog > 0 ? 'NAO_ATENDE' : 'ATENDE';
...
function buildScenarioPoints(
  ...
  if (!dates.length || totalDailyCapacity <= 0) return [];

  const start = parseIsoDate(dates[0]);
  const lastDemandDate = parseIsoDate(dates[dates.length - 1]);
  const totalProgrammed = Array.from(series.values()).reduce(
    (sum, value) => sum + value,
    0
  );
  ...
  const maxSafetyDays = 2000;
  let safety = 0;

  while (safety < maxSafetyDays) {
    const iso = toIsoDate(current);
    const demand = series.get(iso) ?? 0;

    cumulativeDemand += demand;

    if (capacity < cumulativeDemand) {
      capacity = Math.min(capacity + totalDailyCapacity, totalProgrammed);
    }
    ...
    points.push({
      ...
      backlog,
      machinesNeeded:
        totalDailyCapacity > 0 ? Math.ceil(backlog / totalDailyCapacity) : 0,
    });

    if (
      current.getTime() >= lastDemandDate.getTime() &&
      capacity >= totalProgrammed
    ) {
      break;
    }
...
function App() {
  ...
  const [operationType, setOperationType] =
    useState<OperationType>('ADUBACAO');
  const [groupBy, setGroupBy] = useState<GroupBy>('FAZENDA');

  const [varietyConfig, setVarietyConfig] = useState<
    Record<string, VarietyConfig>
  >({});

  const [operations, setOperations] = useState<OperationItem[]>([
    {
      id: crypto.randomUUID(),
      product: 'Primeira operação',
      dae: 5,
    },
  ]);
  ...
  const varieties = useMemo(
    () => Array.from(new Set(fieldSummaries.map((item) => item.variedade))).sort(),
    [fieldSummaries]
  );
  ...
  const fieldsWithDates = useMemo(() => {
    return fieldSummaries
      .map((field) => {
        ...
        if (
          !config ||
          !config.germinationDays ||
          !config.cycleDays ||
          config.germinationDays < 0 ||
          config.cycleDays < 0
        ) {
          return null;
        }
      ...
      FieldSummary & { emergenceDate: Date; harvestDate: Date }
    >;
  }, [fieldSummaries, varietyConfig]);

  const totalDailyCapacity = useMemo(() => {
    return machines.reduce((sum, item) => {
      const quantity = Number.isFinite(item.quantity) ? item.quantity : 0;
      const haPerDay = Number.isFinite(item.haPerDay) ? item.haPerDay : 0;
      return sum + quantity * haPerDay;
    }, 0);
  }, [machines]);
  ...
  const scenarios = useMemo<ScenarioResult[]>(() => {
    ...
    fieldsWithDates.forEach((field) => {
      ...
      if (operationType === 'COLHEITA') {
        ...
        operations.forEach((operation) => {
          ...
          pushDemand(
            groupName,
            addDays(field.emergenceDate, operation.dae),
            field.areaTotal
          );
        });
      }
    ...
    return Array.from(scheduleMap.entries())
      .map(([groupName, series]) => {
        const points = buildScenarioPoints(series, totalDailyCapacity);
        if (!points.length) return null;

        const totalArea = points[points.length - 1]?.cumulativeDemand ?? 0;
        const maxBacklog = Math.max(...points.map((p) => p.backlog), 0);
  ...
  const periodsByScenario = useMemo(() => {
    return Object.fromEntries(
      scenarios.map((scenario) => [
        scenario.groupName,
        buildAlertPeriods(scenario.points),
      ])
    );
  }, [scenarios]);
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const uniqueVarieties = Array.from(
        new Set(parsed.map((row) => row.variedade))
      ).sort();
  ...
  const canAdvanceToOperations =
    rawRows.length > 0 &&
    varieties.every((name) => {
    ...
    <div className="page-shell">
      <header className="hero-card">
        <div>
          ...
          <p>
            Faça upload da planilha de plantio, calcule automaticamente a data de
            plantio pelo critério de 70% da área total, projete germinação e
            ciclo e simule a capacidade do parque de máquinas para adubação,
      ...
      <section className="card">
        <div className="section-title">
          ...
          <span>
            {fileName ? `Arquivo carregado: ${fileName}` : 'Envie a base de plantio'}
          </span>
        </div>
        ...
        {rawRows.length > 0 ? (
          <div className="stats-grid">
            ...
            <div className="stat-card">
              ...
              <span>talhões consolidados</span>
            </div>
            <div className="stat-card">
              <strong>{varieties.length}</strong>
              <span>variedades encontradas</span>
            </div>
            <div className="stat-card">
              <strong>
                {formatNumber(fieldSummaries.reduce((sum, item) => sum + item.areaTotal, 0))}
              </strong>
              <span>ha totais</span>
            </div>
          </div>
                  ...
                  <tr key={variety}>
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) =>
                          updateVariety(variety, 'cycleDays', Number(e.target.value))
                        }
                      />
                    </td>
                  </tr>
        ...
        <section className="card">
          ...
          {operationType !== 'COLHEITA' ? (
                  ...
                  <div className="operation-card" key={operation.id}>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateOperation(operation.id, 'product', e.target.value)
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={operation.dae}
                        onChange={(e) =>
                          updateOperation(operation.id, 'dae', Number(e.target.value))
                        }
                      />
                    </div>
            ...
            <div className="info-box">
              Para colheita, a data da demanda é calculada automaticamente como{' '}
              <strong>Emergência + Ciclo total</strong>.
            </div>
          )}
        </section>
      ) : null}
      ...
      {canAdvanceToOperations ? (
        <section className="card">
              ...
              <div className="machine-card" key={machine.id}>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateMachine(machine.id, 'quantity', Number(e.target.value))
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateMachine(machine.id, 'haPerDay', Number(e.target.value))
                    }
                  />
                </div>
          ...
          <div className="stats-grid compact">
            <div className="stat-card">
              <strong>{formatNumber(totalDailyCapacity)}</strong>
              <span>ha/dia total</span>
            </div>
            <div className="stat-card">
              <strong>{machines.reduce((sum, item) => sum + item.quantity, 0)}</strong>
              <span>máquinas totais</span>
            </div>
            <div className="stat-card">
              <strong>{OPERATION_LABEL[operationType]}</strong>
              <span>operação simulada</span>
            </div>
          </div>
        </section>
      ) : null}
      ...
      {scenarios.length > 0 ? (
        <section className="card">
          ...
          <div className="info-box">
            Regra da capacidade: a máquina trabalha somente perante a demanda.
            Assim, a linha de capacidade sobe até alcançar a demanda acumulada e
            fica estável até surgir nova necessidade.
          </div>
            ...
            <div className="scenario-block" key={scenario.groupName}>
              <div className="scenario-header">
                <div>
                  ...
                  <p>
                    Área demandada: <strong>{formatNumber(scenario.totalArea)} ha</strong> •
                    Capacidade diária: <strong>{formatNumber(scenario.dailyCapacity)} ha/dia</strong>
                  </p>
                </div>

                <div
                  className={`badge ${
                    scenario.maxBacklog > 0 ? 'badge-warn' : 'badge-ok'
                  }`}
                >
                  {scenario.maxBacklog > 0
                    ? `Gargalo máximo: ${formatNumber(scenario.maxBacklog)} ha`
                    : 'Capacidade atende a demanda'}
                </div>
              </div>
      ...
      <section className="card muted-card">
        ...
        <ul className="clean-list">
          <li>
            A data de plantio do talhão é definida no primeiro dia em que o
            acumulado de <strong>Área Plantada(ha)</strong> atinge ou ultrapassa{' '}
            <strong>70% da Área Total(ha)</strong>.
          </li>
          <li>
            A data de emergência é calculada como{' '}
            <strong>Data de Plantio + Dias para Germinação</strong>.
          </li>
          <li>
            Para colheita, a demanda acontece em{' '}
            <strong>Emergência + Ciclo total</strong>.
          </li>
          <li>
            Para adubação e pulverização, cada operação usa{' '}
            <strong>Emergência + DAE</strong>.
          </li>
          <li>
            A capacidade acumulada sobe diariamente conforme o parque de máquinas,
            mas nunca ultrapassa a demanda acumulada.
          </li>
          <li>
            O gráfico continua avançando no calendário até o momento em que a
            <strong> capacidade acumulada alcança 100% da programação total</strong>.
          </li>
        </ul>
      </section>
    </div>
